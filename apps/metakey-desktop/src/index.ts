...
import { IpcChannel, Theme, WidgetConfig, DisplayLayout, ThemeLayouts, SpellbookEntry, SpellbookMenuIcon } from 'shared-types';
...
class MainApplication {
  ...
  private registerActions(): void {
    ...
    // Spellbook navigation actions
    this.hotkeyEngine.registerAction('spellbook:nav-up', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'ArrowUp' });
    });
    this.hotkeyEngine.registerAction('spellbook:nav-down', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'ArrowDown' });
    });
    this.hotkeyEngine.registerAction('spellbook:nav-left', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'ArrowLeft' });
    });
    this.hotkeyEngine.registerAction('spellbook:nav-right', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'ArrowRight' });
    });
    this.hotkeyEngine.registerAction('spellbook:select', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'Enter' });
    });
    this.hotkeyEngine.registerAction('spellbook:escape', () => {
      // Send the escape key to the renderer for handling
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'Escape' });
      // Also directly close the spellbook as a fallback
      if (this.isSpellbookVisible) {
        this.toggleSpellbookMode();
      }
    });
    
    // Quick action keys for spellbook menus
    this.hotkeyEngine.registerAction('spellbook:echoes', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'E' });
    });
    this.hotkeyEngine.registerAction('spellbook:themes', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'T' });
    });
    this.hotkeyEngine.registerAction('spellbook:alt-up', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'ArrowUp' });
    });
    this.hotkeyEngine.registerAction('spellbook:alt-down', () => {
      this.broadcast(IpcChannel.SPELLBOOK_NAVIGATE, { key: 'ArrowDown' });
    });
  ...
  private createSpellbookNavigationBindings(): HotkeyBinding[] {
    return [
      // Arrow key navigation
      { shortcut: 'Up', actionId: 'spellbook:nav-up', payload: {} },
      { shortcut: 'Down', actionId: 'spellbook:nav-down', payload: {} },
      { shortcut: 'Left', actionId: 'spellbook:nav-left', payload: {} },
      { shortcut: 'Right', actionId: 'spellbook:nav-right', payload: {} },
      
      // Selection keys - support both Return and Enter
      { shortcut: 'Return', actionId: 'spellbook:select', payload: {} },
      { shortcut: 'Enter', actionId: 'spellbook:select', payload: {} },
      
      // Escape to close
      { shortcut: 'Escape', actionId: 'spellbook:escape', payload: {} },
      
      // Quick action keys for menu navigation
      { shortcut: 'E', actionId: 'spellbook:echoes', payload: {} },
      { shortcut: 'T', actionId: 'spellbook:themes', payload: {} },
      
      // Alternative navigation using WASD-style keys
      { shortcut: 'W', actionId: 'spellbook:alt-up', payload: {} },
      { shortcut: 'S', actionId: 'spellbook:alt-down', payload: {} },
      { shortcut: 'A', actionId: 'spellbook:nav-left', payload: {} },
      { shortcut: 'D', actionId: 'spellbook:nav-right', payload: {} },
    ];
  }
  ...
  private handleKeyEvent(event: SystemAgentEvent, type: 'press' | 'release'): void {
      ...
      // Note: Spellbook navigation is now handled via dynamic hotkey registration
      // No need to intercept raw key events when spellbook is visible
  ...
  private async toggleSpellbookMode(): Promise<void> {
    ...
    if (this.isSpellbookVisible) {
      // Register spellbook navigation hotkeys
      console.log('[Main] Registering spellbook navigation hotkeys...');
      const currentBindings = this.hotkeyEngine.getBindings();
      const spellbookBindings = this.createSpellbookNavigationBindings();
      this.hotkeyEngine.registerBindings([...currentBindings, ...spellbookBindings]);
      ...
      // Unregister spellbook hotkeys - revert to original bindings
      console.log('[Main] Unregistering spellbook navigation hotkeys...');
